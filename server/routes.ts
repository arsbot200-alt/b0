...
async function initializeNewUser(userId: string, username: string, invitationCode?: string) {
  ...
  const defaultAchievements = [
    ...
    },
    {
      ...
      description: 'Invite 10 friends to join PingCaset',
      reward: 50,
      isCompleted: false,
      progress: 0,
      target: 10
    },
    {
      ...
      title: 'Play 50 Games',
      description: 'Play mini-games 50 times',
      reward: 100,
      isCompleted: false,
      progress: 0,
      target: 50
    ...
    if (inviter) {
      ...
      if (inviterProfile) {
        await storage.updateUserProfile(inviter.id, {
          balance: inviterProfile.balance + 25,
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post('/api/spin/:userId', async (req, res) => {
    try {
      ...
      if (profile.balance < 5) {
        return res.status(400).json({ error: 'Not enough coins. Need 5 coins to spin.' });
      }

      const rewardOptions = [
        { value: 0, weight: 0, label: 'Unlucky' },
        { value: 10, weight: 70, label: '10' },
        { value: 20, weight: 20, label: '20' },
        { value: 50, weight: 7, label: '50' },
        { value: 100, weight: 2, label: '100' },
        { value: 500, weight: 1, label: '500' }
      ];
      ...
      const updatedProfile = await storage.updateUserProfile(req.params.userId, {
        balance: profile.balance + reward - 5,
  ...
  app.post('/api/scratch-card/new/:userId', async (req, res) => {
    try {
      ...
      if (!profile || profile.balance < 10) {
        return res.status(400).json({ error: 'Not enough coins. Need 10 coins to play scratch card.' });
      }
  ...
  app.post('/api/scratch-card/:cardId', async (req, res) => {
    try {
      ...
      await updateAchievementProgress(card.userId, 'play_games', 1);
